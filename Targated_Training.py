...
logging.basicConfig(filename='PGD_Targatted_Average_loss_Resnet18.log', level=logging.INFO)
...
def Train(org_loss, loss_class, l2, net, trainloader, optimizer, scheduler, epochs ):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            # Generate targeted labels for misclassification
            target_labels = generate_targeted_labels(labels)

            # Apply PGD attack
            perturbed_data = pgd_targated_attack(net, inputs, target_labels)
            ...
            if loss_class:
                ...
                for j in range(num_classes):
                    ...
                    if l2:
                        ...
                        class_norm = torch.sqrt(torch.sum(class_loss))  # SSQE
...
if __name__ == "__main__":
    ...
    num_epochs = 60
    Balanced = True # True then balanced batch loader (Equal no of samples per batch)
    org_loss = False # True when you want to calculate adversarial + orginal images loss and perform bacwardpass.(Generally we Keep false if loss class is true)
    ...
    torch.save(model.state_dict(), 'PGD_Targatted_Average_loss_Resnet18.pth')

    plot_loss_vs_epoch(train_loss, train_acc ,title="PGD_Targated_Average_loss_Training_Loss_&_Accuracy_vs_Epoch", filename= "PGD_Targated_Average_loss_Training_Loss_&_Accuracy_vs_Epoch.png", )
    ...
    Targated_model = load_model('PGD_Targatted_Average_loss_Resnet18.pth', device)
    ...
    plot_confusion_matrix(all_targets, all_preds, classes, title= "PGD_Targated_Average_loss_Confusion_matrix_Standard_Data", filename="PGD_Targated_Average_loss_Confusion_matrix_Standard_Data.png")
    ...
    plot_confusion_matrix(all_targets, all_preds, classes,title= "PGD_Targated_Average_loss_Confusion_matrix_Untargated_Data", filename="PGD_Targated_Average_loss_Confusion_matrix_Untargated_Data.png")
    ...
    plot_confusion_matrix(all_targets, all_preds, classes,title= "PGD_Targated_Average_loss_Confusion_matrix_Targated_Data",  filename="PGD_Targated_Average_loss_Confusion_matrix_Targated_Data.png")
